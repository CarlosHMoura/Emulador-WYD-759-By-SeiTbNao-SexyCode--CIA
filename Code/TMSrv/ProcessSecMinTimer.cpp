...
#pragma region >> Verifica se o usuário está com várias contas abertas
		...
		/* Batalha Real *//*
		GetUserInAreaMac(0, 197, 213, 215, 230);*/
		...
		/* Área de evento */
		GetUserInAreaMac(0, 1302, 1516, 1381, 1483);
...
#pragma region Itens Time
	if (SecCounter % 120 == 0)
	{
		for (int user = 0; user < MAX_USER; user++)
		{
			...
			if (pMob[user].MOB.Equip[14].sIndex == 5705 && BASE_CheckItemDate(&pMob[user].MOB.Equip[14]))
			{
				sprintf(temp, "etc,premium item end %d", pMob[user].MOB.Equip[14].sIndex);
				Log(temp, "-system", 0);
		...
		{//by[SexyCode]
			...
			if (p_BatalhaReal.start == true)
			{
				...
				for (int i = 0; i < 20; i++)
				{
					...
					if (BrTime > 0)
						BrTime--;  
					SendClientSignalParmCoord(i, ESCENE_FIELD, _MSG_StartTime, BrTime, 197, 213, 215, 230);
				}
			} 
		}
#pragma endregion
		...
		if (SecCounter % 2 == 0)
		{
			...
			for (int i = 0; i < MAX_USER; i++)
			{
				...
				{ 
					...
					SendClientMessage(i, "Você ganhou +1 ponto de Honra.");
...
#pragma region >> Reset Quest
		...
		if (SecCounter % 2 == 0)
		{
			...
			for (int i = 0; i < MAX_USER; i++)
			{
				...
				SendClientSignalParmCoord(i, ESCENE_FIELD, _MSG_StartTime, pMob[i].CheckQuest, 3604, 3604, 3691, 3690);
				SendClientSignalParmCoord(i, ESCENE_FIELD, _MSG_StartTime, pMob[i].CheckQuest, 3732, 3476, 3819, 3563);
				SendClientSignalParmCoord(i, ESCENE_FIELD, _MSG_StartTime, pMob[i].CheckQuest, 3604, 3860, 3946, 3690);
				...
				if (pMob[i].CheckQuest <= 0) // 15 min porque passa de 2 em 2 ms
				{
					...
					ClearAreaQuest(i, 3604, 3604, 3691, 3690);//Lanhouse N  
					ClearAreaQuest(i, 3732, 3476, 3819, 3563);//Lanhouse M  
					ClearAreaQuest(i, 3604, 3860, 3946, 3690);//Lanhouse A 
...
#pragma region Limpa se não for o level da quest
				if (pMob[i].MOB.BaseScore.Level > 115)  
				{
					if (pMob[i].TargetX > 2379 && pMob[i].TargetY > 2076 &&
						pMob[i].TargetX < 2426 && pMob[i].TargetY < 2133)
				...
				if (pMob[i].MOB.BaseScore.Level > 190) 
				{
					if (pMob[i].TargetX > 2228 && pMob[i].TargetY > 1700 &&
						pMob[i].TargetX < 2257 && pMob[i].TargetY < 1728)
				...
				if (pMob[i].MOB.BaseScore.Level > 265)  
				{
					if (pMob[i].TargetX > 459 && pMob[i].TargetY > 3887 &&
						pMob[i].TargetX < 497 && pMob[i].TargetY < 3916)
				...
				if (pMob[i].MOB.BaseScore.Level > 321) 
				{
					if (pMob[i].TargetX > 658 && pMob[i].TargetY > 3728 &&
						pMob[i].TargetX < 703 && pMob[i].TargetY < 3762)
				...
				if (pMob[i].MOB.BaseScore.Level > 351) 
				{
					if (pMob[i].TargetX > 1312 && pMob[i].TargetY > 4027 &&
						pMob[i].TargetX < 1348 && pMob[i].TargetY < 4055)
...
#pragma endregion
		...
		////pista +0
		//if (when.tm_sec > 1 && when.tm_sec < 59)
		//{
		//	for (int i = 0; i < MAX_USER; i++)
		//	{
		//		if (pMob[i].TargetX > 3333 && pMob[i].TargetY > 1601 && pMob[i].TargetX < 3448 && pMob[i].TargetY < 1656/* && pMob[i].MOB.BaseScore.Level > 1*/)
		...
		//	for (int i = 0; i < MAX_USER; i++)
		//	{
		//		if (pMob[i].TargetX > 3347 && pMob[i].TargetY > 1545 && pMob[i].TargetX < 3433 && pMob[i].TargetY < 1591/* && pMob[i].MOB.BaseScore.Level > 1*/)

		//			//	if (mob->PosX > 3347 && mob->PosX < 3433 && mob->PosY > 1545 && mob->PosY < 1591)
		...
		//	for (int i = 0; i < MAX_USER; i++)
		//	{
		//		if (pMob[i].TargetX > 3340 && pMob[i].TargetY > 1425 && pMob[i].TargetX < 3449 && pMob[i].TargetY < 1463/* && pMob[i].MOB.BaseScore.Level > 1*/)

		//			//	if (mob->PosX > 3340 && mob->PosX < 3449 && mob->PosY > 1425 && mob->PosY < 1463)
		...
		//	for (int i = 0; i < MAX_USER; i++)
		//	{
		//		if (pMob[i].TargetX > 3333 && pMob[i].TargetY > 1029 && pMob[i].TargetX < 3448 && pMob[i].TargetY < 1144/* && pMob[i].MOB.BaseScore.Level > 1*/)

		//			//	if (mob->PosX > 3333 && mob->PosX < 3448 && mob->PosY > 1029 && mob->PosY < 1144)
		...
		//	for (int i = 0; i < MAX_USER; i++)
		//	{
		//		if (pMob[i].TargetX > 3334 && pMob[i].TargetY > 1285 && pMob[i].TargetX < 3451 && pMob[i].TargetY < 1402/* && pMob[i].MOB.BaseScore.Level > 1*/)

		//			//	if (mob->PosX > 3334 && mob->PosX < 3451 && mob->PosY > 1285 && mob->PosY < 1402)
		...
		//	for (int i = 0; i < MAX_USER; i++)
		//	{
		//		if (pMob[i].TargetX > 3333 && pMob[i].TargetY > 1157 && pMob[i].TargetX < 3448 && pMob[i].TargetY < 1271/* && pMob[i].MOB.BaseScore.Level > 1*/)

		//			//	if (mob->PosX > 3333 && mob->PosX < 3448 && mob->PosY > 1157 && mob->PosY < 1271)
		...
		//	for (int i = 0; i < MAX_USER; i++)
		//	{
		//		if (pMob[i].TargetX > 3332 && pMob[i].TargetY > 1471 && pMob[i].TargetX < 3449 && pMob[i].TargetY < 1529/* && pMob[i].MOB.BaseScore.Level > 1*/)

		//			//	if (mob->PosX > 3332 && mob->PosX < 3449 && mob->PosY > 1471 && mob->PosY < 1529)
...
#pragma region Start Batalha Real
		...
		if (when.tm_wday != 0 && when.tm_wday != 6 && when.tm_hour == BatalhaRealHour && when.tm_min == 10)
		{
			bool status = false;
			if (status == false)
			{
				if (BrState == 0 && when.tm_sec >= 0 && when.tm_sec <= 10)
				{
					MSG_Action sm;
					if (p_BatalhaReal.NumMembers >= 2)
					{
						...
						for (int i = 0; i < 20; i++)
						{
							...
							DoTeleport(p_BatalhaReal.Members[i].Id, 204, 222);    
...
#pragma region Batalha Real Action

		if (p_BatalhaReal.start == true)
		{ 
			for (int i = 0; i < 20; i++)
			{
				auto player = GetMobFromIndex(i);

				if (!player->PosX >= 197 && player->PosY >= 213 && player->PosX <= 215 && player->PosY <= 230)
				{
					p_BatalhaReal.Members[i].Id -= 1;
					p_BatalhaReal.NumMembers -= 1;
					return;
				}
			}
		}
		if (p_BatalhaReal.NumMembers == 1 && p_BatalhaReal.start == true)
		{
			for (int i = 1; i < MAX_USER; ++i)
			{
				...
				auto players = GetMobFromIndex(i);
				if (!players->PosX >= 197 && players->PosY >= 213 && players->PosX <= 215 && players->PosY <= 230)
				{
					p_BatalhaReal.start = false;
					memset(&p_BatalhaReal, 0x0, sizeof(p_BatalhaReal)); //zerar dados da struct

					DoRecall(i);
					ClearArea(197, 213, 215, 230);

					STRUCT_ITEM presente = { 412, 61, 20, 0 };
					STRUCT_ITEM presente2 = { 413, 0, 0, 0 };
					STRUCT_ITEM presente3 = { 420, 0, 0, 0 };
					STRUCT_ITEM presente4 = { 419, 0, 0, 0 };
					STRUCT_ITEM presente5 = { 4144, 61, 5, 0 };
					PutItem(i, &presente);
					PutItem(i, &presente2);
					PutItem(i, &presente3);
					PutItem(i, &presente4);
					PutItem(i, &presente5);
					SendClientMessage(i, "Parabens, Você venceu a Arena Real.");
					SendCarry(i);
					BrState = 0;
					BrTime = 0;
				}
...
#pragma region End Batalha Real
		...
		{
			//Finaliza a Batalha caso não haja ganhadores.
			p_BatalhaReal.start = false;
			memset(&p_BatalhaReal, 0x0, sizeof(p_BatalhaReal));
			ClearArea(197, 213, 215, 230);
...
#pragma region Avisos
		if ((when.tm_min == 20 || when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 0)
		{
			SendNotice("Bem vindo(a) ao WYD Malech!");
			return;
		}
		if ((when.tm_min == 20 || when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 5)
		{
			SendNotice("Relatem os bugs no site! WYDMALECH.COM.BR");
			return;
		}
		if ((when.tm_min == 20 || when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 10)
		{
			SendNotice("Siga nossa página no Facebook @MalechWYD e fique atualizado!");
			return;
		}
		if ((when.tm_min == 20 || when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 15)
		{
			SendNotice("Digite /comandos para começar!");
			return;
		}
		if ((when.tm_min == 20 || when.tm_min == 40 || when.tm_min == 00) && when.tm_sec == 10)
		{
			SendNotice("Agradecemos a sua preferência. Bom jogo!");
			return;
		}
#pragma endregion
	}
