...
int GenerateSummon(int conn, int SummonID, STRUCT_ITEM *sItem, int Num)
{
	...
	if (MobEmpty == 0)
	{
		 SendMsgExp(conn, g_pMessageStringTable[_NN_Cant_Create_More_Summons], TNColor::Default, false);
		return 0;
	}
	...
	for (int i = SummonNum; i < Num; i++)
	{
		...
		if (j >= MAX_PARTY)
		{
			...
			SendMsgExp(conn, g_pMessageStringTable[_NN_Party_Full_Cant_Summon], TNColor::Default, false);
...
BOOL InitInstance(HANDLE hInstance, int nCmdShow)
{
	...
	hWndMain = CreateWindow("MainClass", "Emulador Open 759 by[SexyCode]",
