	...
	if (conn == 0)
	{
		...
		case _MSG_DBCheckPrimaryAccount:
		{
			...
			for (int i = 1; i < MAX_USER; i++)
			{
				...
				if (strcmp(pUser[i].MacAddress, m->MacAddres))
					continue;
...
#pragma region _MSG_DBCNFAccountLogin
		case _MSG_DBCNFAccountLogin:
		{
			...
			{
				if (pMac[x].MacAddrs[0] == 0)
					continue;

				if (strcmp(pMac[x].MacAddrs, pUser[conn].MacAddress) == 0)
				{
			...
			sprintf(temp, "CNFAccountLogin Mac: %s", pUser[conn].MacAddress);
...
#pragma region _MSG_DBCNFCharacterLogin
		case _MSG_DBCNFCharacterLogin:
		{
			...
			if (KefraLive != 0)
			{
				...
				SendMsgExp(conn, tt, NewWhite, FALSE);
			}
			...
			sprintf(tt, "sta,Login char:%s exp:%llu level:%d conn:%d money:%d, store:%d", pMob[conn].MOB.MobName, pMob[conn].MOB.Exp, pMob[conn].MOB.BaseScore.Level, conn, pMob[conn].MOB.Coin, pUser[conn].Donate, pUser[conn].Honra);
