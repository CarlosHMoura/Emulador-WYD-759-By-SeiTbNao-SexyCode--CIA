...
int GetParryRate(STRUCT_MOB *conn, int attackerdex, int attackerrsv, int attackerparry, int defensordex, int defensorrsv, int defensorparry)
{
	...
	if (conn->CurrentScore.Int > conn->CurrentScore.Dex && conn->CurrentScore.Int > conn->CurrentScore.Str)
	{
...
int GetTeleportPosition(int conn, int *x, int *y)
{
	...
	else if (xv == 2548 && yv == 1740) // Azran to vale
	{
		if (pMob[conn].MOB.Equip[13].sIndex == 3916)
		{
			*x = 2281 + rand() % 3;
			*y = 3688 + rand() % 3;
			SendMsgExp(conn, "Bem vindo ao [Vale]", TNColor::Default, false);
		}
	}
...
int GetCreateMob(int mob, MSG_CreateMob *sm)
{
	...
	sprintf_s(sm->Tab, "Informações da Guild");
		...
		sprintf_s(sm->Tab, "Torne-se Cidadão");

	else if (strcmp((char*)sm->MobName, "Carbuncle Wind") == 0)
		sprintf_s(sm->Tab, "Ajudante Mágico");

	else if (strcmp((char*)sm->MobName, "Mestre Grifo") == 0)
		sprintf_s(sm->Tab, "Te levo para quest Baby");

	else if (strcmp((char*)sm->MobName, "Honor Store") == 0)
		sprintf_s(sm->Tab, "Abra sua Lojinha");

	else if (strcmp((char*)sm->MobName, "Guarda Carga") == 0)
		sprintf_s(sm->Tab, "Baú");

	else if (strcmp((char*)sm->MobName, "Cap.Cavaleiros") == 0)
		sprintf_s(sm->Tab, "Skill Master");

	else if (strcmp((char*)sm->MobName, "ForeLearner") == 0)
		sprintf_s(sm->Tab, "Skill Master");

	else if (strcmp((char*)sm->MobName, "Foema Ancian") == 0)
		sprintf_s(sm->Tab, "Skill Master");
