...
#pragma region Hardcore cythera
		if (dest->sIndex >= 3503 && dest->sIndex <= 3506)
		{
			if (sanc >= 0)
			{
					SendMsgExp(conn, "Impossível refinar.", TNColor::Default, false);
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
			}
		}
	...
	if (Vol == 7)//Poeira de Fada
	{
		...
		else
		{
			...
			if (pMob[conn].CheckGetLevel())
			{
				...
				SendMsgExp(conn, "+++ LEVEL UP +++", TNColor::Default, false);
...
#pragma region Gema Estelar
	...
	CanSave:
		...
		SendMsgExp(conn, g_pMessageStringTable[_NN_Set_Warp], TNColor::Default, false);
...
#pragma region >> Âmagos
	if (Vol == 16)
	{
		...
		{
			SendMsgExp(conn, g_pMessageStringTable[_NN_Mount_Not_Match], TNColor::Red, false);
			SendItem(conn, m->SourType, m->SourPos, item);
			return;
		}
		...
		{
			SendMsgExp(conn, g_pMessageStringTable[_NN_Cant_Upgrade_More], TNColor::Red, false);
			SendItem(conn, m->SourType, m->SourPos, item);
			ProcessAdultMount(conn, 0);
			return;
		}
			...
			|| level >= 100 && dest->sIndex >= 2332 && dest->sIndex < 2360)
		{
			...
			SendMsgExp(conn, g_pMessageStringTable[_NN_Mount_Growth], TNColor::Default, false);
...
#pragma region Divina 7 15 30 Dias
	if (Vol >= 64 && Vol <= 66)
	{
		...
		divineCast.Parm2 = pUser[conn].Slot;
...
#pragma region Removedor de Pergaminho da Transformação
	if (item->sIndex == 3335)
	{
		...
		if (trans)
		{
			...
			SendMsgExp(conn, tmg, TNColor::Default, false);
...
#pragma region Portão Infernal N/M (Ind/Grupo)
	if (Vol == 176)
	{
		...
		{
			SendItem(conn, m->SourType, m->SourPos, item);
			SendClientMessage(conn, "Use em [ Minuto 4 - 29 || Minuto 34 - 59 ].");
			return;
		}
...
#pragma region HardCore Desligado
		if (isHardCore == 0)
		{
			...
			if (pMob[conn].extra.ClassMaster == CELESTIAL && pMob[conn].MOB.CurrentScore.Level < 399)
			{
				SendClientMessage(conn, "Somente personagem nível 400.");
				SendItem(conn, m->SourType, m->SourPos, item);
				return;
...
#pragma region honra
	if (Vol == 189)
	{
		//if (item->sIndex == 3394)
		{
			pUser[conn].Honra += 1;//3272

			SendMsgExp(conn, "Você ativou 1 de Honra.", TNColor::GoldenClaro, false);
...
#pragma region Extração Elmo
	if (item->sIndex == 3021)
	{
		...
		// Recupera o item level da pedra usada
		//int stoneItemGrade = BASE_GetItemAbility(item, EF_ITEMLEVEL);

		//if (stoneItemGrade <= 5)
		//	{
		//		int helmItemGrade = BASE_GetItemAbility(&pMob[conn].MOB.Equip[1], EF_ITEMLEVEL);
		//		int elmomobtype = BASE_GetItemAbility(&pMob[conn].MOB.Equip[1], EF_MOBTYPE);

		//		if (helmItemGrade <= 0 || helmItemGrade >= 5 || helmItemGrade != stoneItemGrade || (elmomobtype != 2 && elmomobtype != 0))
		//		{
		//			SendItem(conn, m->SourType, m->SourPos, item);
		//			return;
		//	}
		...
		pMob[conn].MOB.Equip[1].stEffect[0].cEffect = item->stEffect[0].cEffect;
		pMob[conn].MOB.Equip[1].stEffect[0].cValue = item->stEffect[0].cValue;
	...
	/*else	//Extração de cythera
	{
	int elmo = pMob[conn].MOB.Equip[1].sIndex;

	if (elmo < 3500 || elmo > 3508)
	{
...
#pragma region Extração Peito
	if (item->sIndex == 3022)
	{
		...
		//if (stoneItemGrade <= 5)
		//	{
		//		int breastItemGrade = BASE_GetItemAbility(&pMob[conn].MOB.Equip[2], EF_ITEMLEVEL);
		///		int breastMobType = BASE_GetItemAbility(&pMob[conn].MOB.Equip[2], EF_MOBTYPE);

		///		if (breastItemGrade <= 0 || breastItemGrade >= 5 || breastItemGrade != stoneItemGrade || (breastMobType != 2 && breastMobType != 0))
		//		{
		//			SendItem(conn, m->SourType, m->SourPos, item);
		//			return;
		//		}
		...
		pMob[conn].MOB.Equip[2].stEffect[0].cEffect = item->stEffect[0].cEffect;
		pMob[conn].MOB.Equip[2].stEffect[0].cValue = item->stEffect[0].cValue;
...
#pragma region Extração Calça
	if (item->sIndex == 3023)
	{
		...
		//	if (stoneItemGrade <= 5) //Item abaixo de E
		//	{
		//		int trousersItemGrade = BASE_GetItemAbility(&pMob[conn].MOB.Equip[3], EF_ITEMLEVEL);
		//		int trousersMobType = BASE_GetItemAbility(&pMob[conn].MOB.Equip[3], EF_MOBTYPE);

		//		if (trousersItemGrade <= 0 && trousersItemGrade >= 5 || trousersItemGrade != stoneItemGrade || (trousersMobType != 2 && trousersMobType != 0))
		//		{
		//			SendItem(conn, m->SourType, m->SourPos, item);
		//			return;
		//		}
		...
		pMob[conn].MOB.Equip[3].stEffect[0].cEffect = item->stEffect[0].cEffect;
		pMob[conn].MOB.Equip[3].stEffect[0].cValue = item->stEffect[0].cValue;
...
#pragma region Extração Luva
	if (item->sIndex == 3024)
	{
		...
		//		int gloveItemGrade = BASE_GetItemAbility(&pMob[conn].MOB.Equip[4], EF_ITEMLEVEL);
		//		int gloveMobType = BASE_GetItemAbility(&pMob[conn].MOB.Equip[4], EF_MOBTYPE);
		//
		//		if (gloveItemGrade <= 0 || gloveItemGrade >= 5 || gloveItemGrade != gloveItemGrade || (gloveMobType != 2 && gloveMobType != 0))
		//		{
		//			SendItem(conn, m->SourType, m->SourPos, item);
		//			return;
		//		}
		...
		pMob[conn].MOB.Equip[4].stEffect[0].cEffect = item->stEffect[0].cEffect;
		pMob[conn].MOB.Equip[4].stEffect[0].cValue = item->stEffect[0].cValue;
...
#pragma region Extração Bota
		if (item->sIndex == 3025)
		{
			...
			//	{
			//		//Item abaixo de E
			//		int bootItemGrade = BASE_GetItemAbility(&pMob[conn].MOB.Equip[5], EF_ITEMLEVEL);
			//		int bootMobType = BASE_GetItemAbility(&pMob[conn].MOB.Equip[5], EF_MOBTYPE);

			//		if (bootItemGrade <= 0 || bootItemGrade >= 5 || bootItemGrade != bootItemGrade || (bootMobType != 2 && bootMobType != 0))
			//		{
			//			SendItem(conn, m->SourType, m->SourPos, item);
			//			return;
			//		}
			...
			pMob[conn].MOB.Equip[5].stEffect[0].cEffect = item->stEffect[0].cEffect;
			pMob[conn].MOB.Equip[5].stEffect[0].cValue = item->stEffect[0].cValue;
...
#pragma region Extração Arma
	if (item->sIndex == 3026)
	{
		...
		//if (destextra != 0 && destextra != 7 || destextra != extra || desttype != 2 && desttype != 0)
		//	{
		//		SendItem(conn, m->SourType, m->SourPos, item);
		//		return;
		//	}
		...
		pMob[conn].MOB.Equip[6].stEffect[0].cEffect = item->stEffect[0].cEffect;
		pMob[conn].MOB.Equip[6].stEffect[0].cValue = item->stEffect[0].cValue;
...
#pragma endregion
#pragma region PremioNovato
	if (item->sIndex == 4907)
	{    
		char RPS[3][10] = { "rock", "paper", "scissor" };

		strcpy(pMob[conn].sPremioNovato, RPS[rand() % 3]);
