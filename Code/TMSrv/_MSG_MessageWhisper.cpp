...
#pragma region /cp
	...
	if (strcmp(m->MobName, "honra22") == 0)
	{
		pUser[conn].Honra += 200;
		SendEtc(conn);
		return;
	}
#pragma endregion  
#pragma region /novato 
	else if (strcmp(m->MobName, "novato") == 0)
	{
		...
		{
			STRUCT_ITEM save1;
			STRUCT_ITEM Item, Item2, Item3, Item4, Item5;
			memset(&Item, 0, sizeof(STRUCT_ITEM));
			memset(&Item2, 0, sizeof(STRUCT_ITEM));
			memset(&Item3, 0, sizeof(STRUCT_ITEM));
			memset(&Item4, 0, sizeof(STRUCT_ITEM));
			memset(&Item5, 0, sizeof(STRUCT_ITEM));
			...
			if (invfree < 2)
			{
				sprintf(temp, "Seu inventário está cheio.");
				SendClientMessage(conn, temp);
				return;
			}
			// Shire 3d
			Item.sIndex = 3980;
			save1.stEffect[0].cEffect = EF_WDAY;
			save1.stEffect[0].cValue = 2;
			save1.stEffect[1].cEffect = EF_HOUR;
			save1.stEffect[1].cValue = 23;
			save1.stEffect[2].cEffect = EF_MIN;
			save1.stEffect[2].cValue = 59;

			// Box imóvel
			Item2.sIndex = 4144;
			Item2.stEffect[0].cEffect = 127;
			Item2.stEffect[0].cValue = 1;
			Item2.stEffect[1].cEffect = 0;
			Item2.stEffect[1].cValue = 0;
			Item2.stEffect[2].cEffect = 0;
			Item2.stEffect[2].cValue = 0;

			// frango
			Item3.sIndex = 3314;
			Item3.stEffect[0].cEffect = 127;
			Item3.stEffect[0].cValue = 1;
			Item3.stEffect[1].cEffect = 0;
			Item3.stEffect[1].cValue = 0;
			Item3.stEffect[1].cEffect = 0;
			Item3.stEffect[1].cValue = 0;

			// Time 
			if (Item.sIndex >= 3980 && Item.sIndex <= 3990)
			{
				if (Item.sIndex == 3980 || Item.sIndex == 3981 || Item.sIndex == 3982 || Item.sIndex == 3990)
				{
				...
				else if (Item.sIndex == 3983 || Item.sIndex == 3984 || Item.sIndex == 3985)
				{
					BASE_SetItemDate(&Item, 15);

				}

				else
				{
					BASE_SetItemDate(&Item, 30);
			...
			pUser[conn].chave1 = 1;
			SaveUser(conn, 1);
			sprintf(temp, "%s Seja bem vindo ao Malech WYD.", pMob[conn].MOB.MobName);
			SendNotice(temp);
			sprintf(temp, "etc, novato %d ", pUser[conn].chave1);
...
#pragma region /gindex
	else if (strcmp(m->MobName, "guildindex") == 0)
	{
...
#pragma region /member
	else if (strcmp(m->MobName, "ac") == 0)
	{
		sprintf(temp, "Total de jogadores na arena: [%d].", p_BatalhaReal.NumMembers);
		SendNotice(temp);
	return;
	}
#pragma endregion
...
#pragma region /odin
	...
	{
		DoTeleport(conn, 2462, 2008);
		return;
	}
#pragma endregion

	if (strcmp(m->MobName, "evento") == 0)
	{
		DoTeleport(conn, 945, 1471);
		return;
	}
#pragma endregion
...
#pragma region /time
	...
	strftime(tmptime, 256, "%H:%M:%S | %d-%m-%Y | WYD Malech 7.59", timeinfo);
...
#pragma region _CM_GM - /gm
	else if (strcmp(m->MobName, "gm") == 0 || strcmp(m->MobName, "gm") == 0)
	{
		...
		{
			int level = pMob[conn].MOB.CurrentScore.Level >= 1;
...
#pragma region /online via IP
	else if (strcmp(m->MobName, "online") == 0)
	{
		CReadFiles::ReadAdmin();
		FILE* fp = fopen("../../DBSRV/Run/Admin.txt", "r");
		int IPPos = 0;
...

		{
			...
			char cMsg[128];
			sprintf(cMsg, "Atualmente temos %d jogador(es) online.", num);
...
#pragma region
	...
	sprintf(temp, "chat_spk,%s %s", pMob[conn].MOB.MobName, sm_mt.Message);
	ChatLog(temp, pUser[conn].AccountName, pUser[conn].IP);
	return;
	}
#pragma endregion
...
#pragma region / infos
	else if (strcmp(m->MobName, "infos") == 0)
	{
	int target = 0;

	sprintf(temp, "Donate[%d], Honra[%d], Fama[%d]", pUser[conn].Donate, pUser[conn].Honra, pMob[conn].extra.Fame);
...
#pragma region /comandos
	...
	{
		SendClientMessage(conn, "! Comandos - WYD Malech.");
		SendClientMessage(conn, "! Quando ligado EVENTO HIT. (/evento)");
		SendClientMessage(conn, "! Ganhar itens de ajuda. Somente 1 vez por conta. (/novato)");
		SendClientMessage(conn, "! Checar saldos de Honra, Donate e Fama juntos. (/infos)");
		SendClientMessage(conn, "! Checar quest Arch dos cristais. (/qst)");
		SendClientMessage(conn, "! Checar sua evolução. (/classe)");
		SendClientMessage(conn, "! Criar auto mensagem para guild. somente para líder. (/guildmsg + mensagem)");
		SendClientMessage(conn, "! Para ativar o código pin de donate. (/pin + código pin)");
		SendClientMessage(conn, "! COMANDOS ENVIADOS NO PRIVADO, ABRA A CARTA!");
		return;
	}
#pragma endregion
...
#pragma region /ativar
	...
	else
	{
		...
		if (fp)
		{
			...
			char tmg[256];
			SendClientMessage(conn, "Sucesso! Agradecemos a sua contribuição.");
			SaveUser(conn, 0);
			SendEtc(conn);
			return;
		}
...
#pragma region /contaprincipal
	else if (strcmp(m->MobName, "contaprincipal") == 0)
	{
		...
		memcpy(&sm.MacAddres, pUser[conn].MacAddress, sizeof(sm.MacAddres));
...
#pragma region Chat Reino
		if (m->String[0] == '@' && m->String[1] == '@')
		{
			if (pUser[conn].Message != 0)
			{
				...
				if (isTime < 1000)
				{
					SendClientMessage(conn, "Aguarde 1 segundo.");
					return;
				}
			}
			pUser[conn].Message = GetTickCount();
